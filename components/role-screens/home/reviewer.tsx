...
export default function ReviewerDashboard() {
  ...
  const loadData = async () => {
    try {
      ...
      const assigned = assignedRaw.map((raw: any) => mapRawToReview(raw));
      const reviewNeeded = reviewRaw.map((raw: any) => mapRawToReview(raw));
      const pending = pendingRaw.map((raw: any) => mapRawToReview(raw));
