...
export default function TaskDetailScreen() {
	...
	const TaskIcon = () => {
		...
		switch (task.task_type) {
			...
			case 'IMAGE':
				return (
					<MaterialCommunityIcons name="file-image" size={24} color="#F97316" />
				);
			case 'VIDEO':
				return (
					<MaterialCommunityIcons name="file-video" size={24} color="#F97316" />
				);
			default:
				return null;
		}
	...
	const StatusIcon = () => {
		...
		switch (task.status) {
			case 'PENDING':
				return (
					<MaterialCommunityIcons name="clock" size={24} color="#eab308" />
				);
			case 'PROCESSING':
				return (
					<MaterialCommunityIcons name="clock" size={24} color="#a855f7" />
				);
			case 'COMPLETED':
				return (
					<MaterialCommunityIcons
	...
	return (
		<SafeAreaView className="flex-1 bg-background">
			<View className="flex-row items-center px-4 py-4 border-b border-border">
				...
				<Text className="text-xl font-bold text-center flex-1 text-foreground">
					Task Details
				</Text>
			</View>
			...
			<ScrollView className="flex-1 p-4">
				<View className="bg-card rounded-lg border border-border p-5 mb-6">
					<View className="flex-row items-center mb-4">
						<TaskIcon />
						<Text className="text-xl font-bold text-foreground ml-2 p-3">
							{task.serial_no}
						</Text>
					</View>
					...
					<View className="space-y-4">
						<View className="flex-row justify-between">
							<View>
								...
								<View className="flex-row items-center mt-1">
									...
									<Text
										className={cn(
											'ml-1 text-white font-medium px-2 py-1 rounded-md'
										)}
									>
						...
						<View>
							<Text className="text-sm text-muted-foreground mb-2 mt-3">
								Human Reviewed
							</Text>
							<Text className="mt-1 text-foreground">
								{task.human_reviewed ? 'Yes' : 'No'}
							</Text>
						</View>
					</View>
				</View>
