...
const fetchTasks = async (accessToken: string, refreshToken: string) => {
  try {
    ...
    if (response.status === 401) {
      ...
      if (newAccessToken) {
        return fetchTasks(newAccessToken, refreshToken); 
      }
    }
...
const TaskHistoryScreen = () => {
  ...
  return (
    <SafeAreaView className="flex-1 bg-background">
      <View className="flex-row items-center px-4 py-4 border-b border-border bg-orange-500">
      ...
      <ScrollView className="p-4">
        {tasks.map((task) => (
          ...
          >
            ...
            />
            <View className="ml-3">
              <Text className="text-lg font-medium text-foreground">{task.title}</Text>
              <Text className="text-sm text-muted-foreground">Serial: {task.serial_no}</Text>
            </View>
          </TouchableOpacity>
        ))}
      </ScrollView>
